...
namespace WhacAMole.Scripts.Audio
{
    ...
    public class ReadOnly2DArray<T> : IReadOnlyCollection<T>
    {
        ...
        public int Rows => m_Array.GetLength(0);
        public int Columns => m_Array.GetLength(1);
        public int Count => m_Array.Length;

        // Indexer to access elements in 2D
        public T this[int row, int col] => m_Array[row, col];

        // Method to flatten the 2D array into a 1D array
        public T[] Flatten()
        {
            T[] flat = new T[Count];
            int index = 0;
            for (int i = 0; i < Rows; i++)
            {
                ...
                {
                    flat[index++] = m_Array[i, j];
                }
            }
            return flat;
        }

        // Flatten the 2D array via IEnumerable<T>
    ...
    public class AudioBuffer
    {
        ...
        // Internal field with a valid name.
        protected float[,] m_Samples;
        // Public read-only property.
        public ReadOnly2DArray<float> Samples => new ReadOnly2DArray<float>(m_Samples);
        /// <summary>
